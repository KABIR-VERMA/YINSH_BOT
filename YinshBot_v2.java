...
class YinshBot_v2 {
	...
	static public int[] removingRingGreedly(String[][] curr_board, int player_no) {
		...
		if (player_no == 1) {
			...
			for (int i = 0; i < ring_arr.size(); i++) {
				...
				int u = Utility(ringremovedboard);
				if (i == 0) {
					util = u;
	...
	// --------------------------------------------------------------------------------
	public static double Gamescore(String[][] curr_board){
		int Rring=0;
		int Bring=0;
		int Rmarker=0;
		int Bmarker=0;
	...
	public static int evalDiscscore(String[][] curr_board){
		int a = 0;
		a = a+ Discinrowver(curr_board);
		///left run
		...
		a1[0]= 6;a1[1]=0 ;
		a2[0]= 0;a2[1]=6 ;
		a = a+ Discindiag(curr_board, a1, a2);
		a1[0]= 8;a1[1]=0 ;
		a2[0]= 1;a2[1]=7 ;
		a = a+ Discindiag(curr_board, a1, a2);
		a1[0]= 10;a1[1]=0 ;
	...
	public static int Discindiag(String[][] curr_board,int[] a1, int[] a2){
		int diag1_score=0;
		if(my_player_no== 1){
			diag1_score=0;
			int[] n = {0,0,0,0,0};
	...
	public static int Discinrowver(String[][] curr_board){
		int ver_score=0;
		if(my_player_no ==1 ){
			ver_score=0;
	...
	public static int Centrering(String[][] curr_board){
	...
	public static int Less_rings_the_better(String[][] gboard){
	...
	public static int whatifNoRings(String[][] gboard){
		String[][] temp_board=getCopyOfboard(gboard);
	...
	public static int mobility(String[][] gboard){
		// :( didn't work well
		// String[][] temp_board=getCopyOfboard(gboard);
		Vector<int[]> pos_of_Rings1 = getPositionOfRing(my_player_no,gboard);
		Vector<int[]> pos_of_Rings2 = getPositionOfRing(opp_player_no,gboard);
		int myscore=0;
		int opp_score=0;
		for (int i = 0; i < pos_of_Rings1.size(); i++) {
			Vector<int[]> validPos = getValidPosofTheRing(gboard, pos_of_Rings1.get(i)[0], pos_of_Rings1.get(i)[1]);
			myscore+=validPos.size();
		}
		for (int i = 0; i < pos_of_Rings2.size(); i++) {
			Vector<int[]> validPos = getValidPosofTheRing(gboard, pos_of_Rings2.get(i)[0], pos_of_Rings2.get(i)[1]);
			opp_score+=validPos.size();
		}


		return(myscore-opp_score);

	}
	
	public static int Utility(String[][] curr_board) {
		// for player us
		int util=0;
		int util1=0;
		int cring1=0;
		...
		util1 = (int) (Gamescore(curr_board)); //- Gamescore(curr_board, opp_player_no));
		cring1 = Centrering(curr_board); //- Centrering(curr_board, opp_player_no) ;
		seq1 = evalDiscscore(curr_board);// - evalDiscscore(curr_board, opp_player_no);
		// runs_count=no_of_Runs(curr_board,player_no);
		less_rings=Less_rings_the_better(curr_board);
		whatif=whatifNoRings(curr_board);
		// mobi=mobility(curr_board);
	
		// System.err.println("-------------------------------");
		// System.err.println("util1 "+util1);
		// System.err.println("cring1 "+cring1);
		...
		// System.err.println("mobility : "+mobi);
		util = 10*util1+13*cring1+2*seq1+4000*less_rings+501*whatif;
		// util = 300*util1+7*cring1+30*seq1+3600*less_rings+500*whatif;
		return util;
	}	
    // --------------------------------------------------------------------------------
    ...
    static public String minmax(String[][] curr_board, int player_no,int depth, int alpha, int beta,int cutoff){
		...
		String tmp="";
       	...
       	if (getPositionOfRing(tpp,curr_board).size()>=3 && getPositionOfRing(player_no,curr_board).size()>=3 ) {
        	// miniMaxAlphaBeta(curr_board, player_no, depth, alpha, beta,cutoff);
        	NegaMax(curr_board, player_no, depth,1,alpha,beta,cutoff);
        	// PVS(curr_board, player_no, depth,1,alpha,beta,cutoff);
       	}
	...
	public static  int miniMaxAlphaBeta(String[][] curr_board, int player_no,int depth, int alpha, int beta,int cutoff) {
		...
		if (gameOver(curr_board) == true) {
			return Utility(curr_board);// game is over
		}

		if(isTerminal(curr_board) == true || depth == cutoff) {
			return Utility(curr_board);
		}
	...
	public static int NegaMax(String[][] curr_board, int player_no, int depth, int color, int alpha, int beta,int cutoff) {
		if (gameOver(curr_board) == true) {
			return color*Utility(curr_board);// game is over
		}
		if(isTerminal(curr_board) == true || depth == cutoff) {
			return color*Utility(curr_board);
		}
		int temp_p=0;
		if (player_no==1) {
			temp_p=2;
		...
		int v = Integer.MIN_VALUE;
		Vector<Pair<String[][], String>> neighbours = AllSingleMoves(curr_board,player_no,false,"");
		for (int i = 0; i < neighbours.size(); i++) {
			int u = Math.max(v, -NegaMax(neighbours.elementAt(i).getFirst(), temp_p, depth+1,-color,-beta,-alpha,cutoff));
			if (u > v) {
				if (depth==0) {
					returnMove = neighbours.elementAt(i).getSecond();
				}
	...
	public static int PVS(String[][] curr_board, int player_no, int depth, int color, int alpha, int beta,int cutoff){
		if (gameOver(curr_board) == true) {
			return color*Utility(curr_board);// game is over
		}
		if(isTerminal(curr_board) == true || depth == cutoff) {
		...
		for (int i = 0; i < neighbours.size(); i++) {
			int score=-1;
			if (i==0) {
				score= -NegaMax(neighbours.elementAt(i).getFirst(), temp_p, depth+1,-color,-beta,-alpha,cutoff);
			}
			else{
				score= -NegaMax(neighbours.elementAt(i).getFirst(), temp_p, depth+1,-color,-alpha-1,-alpha,cutoff);
				if (alpha< score && score<beta) {
					score=-NegaMax(neighbours.elementAt(i).getFirst(), temp_p, depth+1,-color,-beta,-score,cutoff);	
				}
			}
	...
	// public static void OrderStates(Vector<Pair<String[][], String>> states){
	...
	public static String get_move() {
		...
		String s = minmax(board, my_player_no, 0, Integer.MIN_VALUE, Integer.MAX_VALUE,4);
...
class Pair<F, S> implements Comparable<Pair<String[][], String>> {
    ...
    @Override
    public int compareTo(Pair<String[][],String> p) {
    	// YinshBot_v2.Utility(p.getFirst(),)
        return 0;
    }
}